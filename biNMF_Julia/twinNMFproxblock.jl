...
function twinNMFproxblock(X1,X2,P1,P2,E1,E2,A,lambda_correlation,lambda_sparsity,max_iter, alpha, eps, tol)
    f = zeros(max_iter,1)
    ...
    while (diff >= f[1]*tol && counter < max_iter)
        ...
        E1 = E1 - 2*step*(transpose(P1)*(-X1 + P1*E1) + lambda_correlation*grad_E1(A,E1,E2))
        E1 = projpositive(E1)
 ...
 =#
        ...
        E2 = E2 - 2*step*(transpose(P2)*(-X2 + P2*E2) + lambda_correlation*grad_E2(A,E1,E2)) 
        E2 = projpositive(E2)
